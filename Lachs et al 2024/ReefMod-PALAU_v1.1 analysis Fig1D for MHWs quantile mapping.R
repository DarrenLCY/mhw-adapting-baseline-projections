...
# compute average summer temperatures (to implement niche conservatism) -----
library(dplyr)
library(lubridate)
library(zoo)

ts_clim <- ts %>%
  filter(format(t, "%m-%d") != "02-29") %>%   # remove leap day
  mutate(doy = yday(t)) %>%
  group_by(doy) %>%
  summarise(
...
summer_center_doy <- clim_circ$doy[which.max(clim_circ$roll_sum)]
summer_center_doy <- ((summer_center_doy - 1) %% 365) + 1

get_window <- function(center, half_width) {
  ((center - half_width):(center + half_width - 1) - 1) %% 365 + 1
}

summer_window_doy <- get_window(
  center = summer_center_doy,
  half_width = half_window
)

ggplot(ts_clim, aes(doy, seas)) +
  geom_line(color = "grey40") +
  geom_vline(xintercept = summer_center_doy, color = "red", linetype = "dashed") +
  labs(
    x = "Day of year",
    y = "Climatological SST (°C)",
    title = "Climatological summer defined by maximum 12-week SST sum"
  ) +
  theme_classic()

# this region in Palau has 2 summer peaks, so let's just compute the ast during the period that covers the 2 peaks

ggplot(ts_clim, aes(doy, seas)) +
  geom_line(color = "grey40") +
  geom_vline(xintercept = 123, color = "red", linetype = "dashed") +
...
ts_clim_filtered = ts_clim[which(ts_clim$doy >= 123 & ts_clim$doy <= 350),]
mean(ts_clim_filtered$seas)

# end: compute average summer temperatures ----------------- 
...
BMI$BMI = 1-BMI$BMI # or you can simply do this... 
...
hist(HT$DHW50, main = NULL, xlab = expression(CI[50]~"(" * degree * "C-weeks)"))
mean(HT$DHW50)
var(HT$DHW50)^0.5
summary(HT$DHW50)
quantile(HT$DHW50, probs = 0.10, na.rm = TRUE) # 10th perc 

setwd("C:/Users/dlcyli/OneDrive/PhD_Code/SLiM/Lachs et al 2024/")

png("CI50_hist.png", height = 10, width = 15, units = "cm", res = 300)
hist(HT$DHW50, main = NULL, xlab = expression(CI[50]~"(" * degree * "C-weeks)"))
dev.off()
...
setwd("C:/Users/dlcyli/OneDrive/PhD_Code/SLiM/Lachs et al 2024/")
ggsave("BSP_plot_v2.png", plot = P, width = 6, height = 4, dpi = 300)

# write.table(t(ND2), file = "C:/Users/dlcyli/OneDrive/PhD_Code/SLiM/Main codes/bsi_given_dhw50_res0.05.txt", sep = "\t", row.names = FALSE, col.names = FALSE)
write.table(t(ND2), file = "C:/Users/dlcyli/OneDrive/PhD_Code/SLiM/Main codes/bsi_given_dhw50.txt", sep = "\t", row.names = FALSE, col.names = FALSE) # update: 23 Sep 2025 (trying to make the model run faster)
test = read.table("C:/Users/dlcyli/OneDrive/PhD_Code/SLiM/Main codes/bsi_given_dhw50.txt")
# write.table(t(test), file = "C:/Users/dlcyli/OneDrive/PhD_Code/SLiM/Main codes/bsi_given_dhw50.txt", sep = "\t", row.names = FALSE, col.names = FALSE)

# pdf("All colony heat tolerances.pdf", height = 3.5, width=6)
P
# dev.off()
